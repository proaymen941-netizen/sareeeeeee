...
export default function MenuItemCard({ 
  ...
  const isItemFavorite = favoriteStatus?.isFavorite || isLocalMealFavorite(item.id);

  const toggleFavorite = useMutation({
    mutationFn: async () => {
      if (item.isBannerOffer) return;
      const isNowFav = await toggleMealFavoriteWithApi(item.id, user?.id);
      return isNowFav;
    },
    onSuccess: (isNowFav) => {
      if (item.isBannerOffer) return;
      ...
      toast({
        ...
        description: isNowFav ? `تم إضافة ${item.name} إلى قائمة مفضلاتك` : `تمت إزالة ${item.name} من قائمة مفضلاتك`,
      });
    },
  });
