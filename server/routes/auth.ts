...
router.post('/send-otp', async (req, res) => {
  try {
    ...
    if (purpose === 'register') {
      ...
      if (!existingUser) {
        return res.status(404).json({
          success: false,
          message: 'رقم الهاتف غير مسجل في النظام. يرجى التأكد من الرقم أو إنشاء حساب جديد.'
        });
      }
    }
    ...
    let waMsgText = `مرحباً بك في السريع ون 🛵\nرمز التحقق الخاص بك هو: ${code}\nيرجى إدخاله لإنشاء حسابك بنجاح.`;
    if (purpose === 'forgot_password' || purpose === 'reset_password') {
      waMsgText = `مرحباً بك في السريع ون 🔐\nرمز استعادة كلمة المرور الخاص بك هو: ${code}\nيرجى إدخاله لتعيين كلمة مرور جديدة لحسابك.`;
    }
    const waMessage = encodeURIComponent(waMsgText);
    ...
    console.log(`📱 [OTP] القناة المفعلة: ${otpChannel} | الغرض: ${purpose} | الرقم ${phone} | الكود: ${code}`);
...
// استعادة وتغيير كلمة المرور للعميل
router.post('/reset-password', async (req, res) => {
  try {
    ...
    const rawPhone = req.body?.phone || '';
    const rawOtpCode = req.body?.otpCode || '';
    ...
    if (!newPassword || newPassword.length < 3) {
      return res.status(400).json({ success: false, message: 'كلمة المرور الجديدة يجب أن لا تقل عن 3 أحرف' });
    }
    ...
    if (isOtpEnabled && otpChannel !== 'disabled') {
      ...
      if (!otpRecord) {
        return res.status(400).json({
          success: false,
          message: 'لم يتم طلب رمز تحقق لهذا الرقم أو انتهت صلاحيته. يرجى طلب رمز جديد.'
        });
      }
      ...
      if (!otpRecord.verified && otpRecord.code !== otpCode) {
        return res.status(400).json({
          success: false,
    ...
    const token = user.id;
    console.log('🔑 تم استعادة وتغيير كلمة المرور بنجاح للمستخدم:', user.name, user.phone);
    ...
    res.json({
      ...
      message: 'تم تغيير كلمة المرور وتعيينها بنجاح'
    });
  } catch (error: any) {
    console.error('خطأ في استعادة كلمة المرور:', error);
    res.status(500).json({
      success: false,
      message: 'حدث خطأ في الخادم أثناء استعادة كلمة المرور'
    });
  }
});
