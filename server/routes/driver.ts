...
router.get("/app/dashboard", requireDriverAuth, async (req: AuthenticatedRequest, res) => {
  try {
    ...
    const availableWasalni = wasalniList
      .filter((r: any) => !['delivered', 'cancelled', 'completed'].includes(r.status) && (!r.driverId || r.driverId === driverId))
      .map((r: any) => ({
        id: r.id,
    ...
    const availableOrders = [
      ...allOrders.filter(order => !['delivered', 'cancelled', 'completed'].includes(order.status) && (!order.driverId || order.driverId === driverId)),
      ...availableWasalni
    ].slice(0, 10);
...
router.get("/orders/available", requireDriverAuth, async (req: AuthenticatedRequest, res) => {
  try {
    ...
    // الطلبات غير المعينة فقط (بدون سائق) أو المعينة لهذا السائق تحديداً
    const availableOrders = allOrders.filter(order => {
      const isNotDone = !['delivered', 'cancelled', 'completed'].includes(order.status);
      const isMyOrUnassigned = !order.driverId || order.driverId === driverId;
      return isNotDone && isMyOrUnassigned;
    });
...
router.get("/wasalni", requireDriverAuth, async (req: AuthenticatedRequest, res) => {
  try {
    ...
    if (status === 'available') {
      // جميع طلبات وصل لي غير المعينة أو المعينة لهذا السائق فقط
