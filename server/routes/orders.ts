...
import { autoAssignDriverToOrder } from "../utils/auto-assign";
...
router.post("/", async (req, res) => {
  try {
    ...
    try {
      ...
      if (storeStatus === 'closed') {
        ...
        if (!appIsOpen) {
          ...
          const msg = isBeforeOpen
            ? `التطبيق مغلق حالياً، يفتح في تمام الساعة ${openingTime}`
            : `التطبيق مغلق حالياً، انتهت ساعات العمل الرسمية (${openingTime} - ${closingTime})`;
          return res.status(400).json({
            error: msg,
            code: "APP_CLOSED",
            message: msg
    ...
    // تحديد اسم وهاتف المطعم/المتجر بشكل كامل ومضمون
    let resolvedRestaurantName = req.body.restaurantName || null;
    if (!resolvedRestaurantName && restaurant && restaurant.name) {
      resolvedRestaurantName = restaurant.name;
    }
    if (!resolvedRestaurantName && restaurantId) {
      try {
        const r = await storage.getRestaurant(restaurantId);
        if (r && r.name) resolvedRestaurantName = r.name;
      } catch (e) {}
    }
    if (!resolvedRestaurantName && itemsString) {
      try {
        const parsedItems = typeof items === 'string' ? JSON.parse(items) : items;
        if (Array.isArray(parsedItems) && parsedItems[0] && parsedItems[0].restaurantName) {
          resolvedRestaurantName = parsedItems[0].restaurantName;
        }
      } catch (e) {}
    }
    if (!resolvedRestaurantName) {
      resolvedRestaurantName = 'المتجر الرئيسي';
    }
    ...
    // إذا تم تحديد سائق محدد مسبقاً (مثلاً من قبل الإدارة) يتم تعيينه
    if (req.body.driverId) {
      try {
        order.driverId = req.body.driverId;
        order.status = 'assigned';
        await storage.updateOrder(order.id, { driverId: req.body.driverId, status: 'assigned' });
      } catch (assignErr) {
        console.error('خطأ في تعيين السائق المخصص:', assignErr);
      }
    } else {
      // توجيه الطلبات إلى جميع السائقين المتاحين (Pool) ليتمكن أي سائق من استلامها
      order.driverId = null;
    ...
    try {
      ...
      await storage.createNotification({
        ...
        message: `طلب جديد رقم ${orderNumber} من ${customerName} (بانتظار استلام السائقين). الموقع: ${deliveryAddress}`,
        recipientType: 'admin',
        recipientId: null,
        orderId: order.id,
        isRead: false
      ...
      if (customerId || customerPhone) {
        await storage.createNotification({
          ...
          message: `تم استلام طلبك رقم ${orderNumber} وهو قيد المراجعة حالياً`,
      ...
      await storage.createOrderTracking({
        orderId: order.id,
        status: orderStatus,
        message: 'تم استلام الطلب وجاري التوصيل تلقائياً للسائقين',
        createdBy: 'system',
      ...
      const storeOrderPayload = {
        ...
        restaurantName: restaurant?.name || 'المتجر الرئيسي',
        createdAt: order.createdAt,
        timestamp: new Date()
      };

      broadcastEvent('new_order', { orderId: order.id, orderNumber: order.orderNumber, status: orderStatus });
      broadcastEvent('new_order_available', storeOrderPayload);

      const ws = (req.app.get('ws') as any);
      if (ws && typeof ws.broadcast === 'function') {
        ws.broadcast('new_order_available', storeOrderPayload);
      }

      // إذا تم التعيين التلقائي، أرسل إشعاراً مباشراً للسائق المُعيّن عبر WebSocket
      if (order.driverId && order.status === 'assigned') {
        try {
          if (ws && typeof ws.sendToDriver === 'function') {
            ws.sendToDriver(order.driverId, 'new_order_assigned', {
              orderId: order.id,
              orderNumber: order.orderNumber,
              autoAssigned: true,
              message: `تم تعيين الطلب ${order.orderNumber} لك تلقائياً`
            });
          }
  ...
  try {
    ...
    if (driverId && available !== 'true') {
      orders = orders.filter(order => order.driverId === driverId && 
        ['assigned', 'pending', 'confirmed', 'preparing', 'ready', 'picked_up', 'on_way'].includes(order.status));
    }
    // فلترة الطلبات المتاحة لجميع السائقين (الطلبات النشطة التي لم تُعيّن بعد لسائق أو معينة لهذا السائق)
    else if (available === 'true') {
      orders = orders.filter(order => 
        !['delivered', 'cancelled', 'completed'].includes(order.status) &&
        (!order.driverId || (driverId && order.driverId === driverId))
      );
    }
...
router.put("/:id/assign-driver", async (req, res) => {
  try {
    ...
    // بث الحدث لجميع متلقي WebSocket (السائقين والإدارة) لإزالة الطلب من قائمة المتاحة عند بقية السائقين
    try {
