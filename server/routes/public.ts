...
router.get("/special-offers", async (req, res) => {
  try {
    ...
    if (restaurantId) {
      offers = offers.filter((o: any) => !o.restaurantId || o.restaurantId === restaurantId);
    }

    if (offerType) {
      offers = offers.filter((o: any) => o.offerType === offerType);
    }
...
router.post("/coupons/validate", async (req, res) => {
  try {
    const { code, orderValue, categoryIds, restaurantId } = req.body;
    ...
    // التحقق من تخصص الكوبون لمجر معين
    if (result.coupon?.restaurantId && restaurantId) {
      if (String(result.coupon.restaurantId) !== String(restaurantId)) {
        return res.json({
          valid: false,
          message: "هذا الكوبون مخصص لمتجر آخر ولا يمكن استخدامه لهذا الطلب"
        });
      }
    }
...
// Verify Android Mobile App API Key
router.get("/verify-app-key", async (req, res) => {
  try {
    const apiKey = (req.headers['x-api-key'] || req.headers['x-app-api-key'] || req.query.apiKey) as string;
    if (!apiKey) {
      return res.status(401).json({ valid: false, message: "مفتاح API مطلوب" });
    }

    const custSetting = await storage.getUiSetting('sec_customer_api_key');
    const driverSetting = await storage.getUiSetting('sec_driver_api_key');

    const customerKey = custSetting?.value;
    ...
    res.status(500).json({ valid: false, message: "خطأ في الخادم" });
  }
});
